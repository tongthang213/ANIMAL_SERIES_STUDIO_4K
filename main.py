...
# Sử dụng class Alignment cố định để tránh lỗi version Flet
...
def main(page: ft.Page):
    ...
    page.title = "Animal Video Studio - Final Edition"
    ...
    def create_video_row(video_val="", title_val="", desc_val="", theme_val="Rừng", label_val=""):
        f1 = ft.TextField(
            label="Video (.mp4 hoặc Link YouTube)",
            width=350,
            value=video_val,
        ...
        f3 = ft.TextField(label="Mô tả", expand=True, value=desc_val, on_change=on_change_handler)
        f_label = ft.TextField(label="Nhãn", width=150, value=label_val if label_val else theme_val,
                               on_change=on_change_handler)
        f_theme = ft.Dropdown(
    ...
    def generate_html_content():
        ...
        # Cỡ chữ nhỏ gọn (đã tối ưu theo ảnh bạn gửi)
        if is_vertical:
            size_title, size_desc, size_tag = "50px", "16px", "14px"
            pos_left, width_content = "30px", "80%"
        else:
            size_title, size_desc, size_tag = "75px", "20px", "16px"
        ...
        dynamic_style = f'''
            ...
            .content {{ 
                ...
                color: #fff; z-index: 10; width: {width_content}; padding: 35px;
                border-radius: 20px; background: rgba(0,0,0,0.3); backdrop-filter: blur(12px);
    ...
    def deploy(e):
        ...
        try:
            ...
            subprocess.run(["git", "commit", "-m", "Final stable build"], check=True)
            subprocess.run(["git", "push", "origin", "main"], check=True)
            page.snack_bar = ft.SnackBar(ft.Text("Đã cập nhật website video thành công!"))
    ...
    page.add(
        ft.Column([
            ft.Container(
                content=ft.Text("ANIMAL VIDEO STUDIO 4K", size=35, weight="bold", font_family="Montserrat",
                                color="orange"),
